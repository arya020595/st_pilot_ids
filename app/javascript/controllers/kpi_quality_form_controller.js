...
// Computes weighted quality scores for KPI step-2 and applies position-based field rules.
export default class extends Controller {
  ...
  connect() {
    ...
    this.applyPositionRules(this.positionValue || "")
  ...
  applyResearchOfficerScoreDisplay() {
    ...
    this.qualitySections().forEach((section) => {
  ...
  updateWeightedScores() {
    ...
    if (output) {
      output.textContent = overall.toFixed(2)
    }
  }

  qualitySections() {
    return this.element.querySelectorAll(".kpi-section-block")
  }

  qualityInputs() {
    return this.element.querySelectorAll("input.score-input")
  }
